# 💼 Automation Engineer Portfolio  RPA + Python + GenAI
# 🤖 RPA + GenAI Projects by Vijay Chavda

Welcome to my automation portfolio! I'm an **Intelligent Automation Engineer** with **3.9+ years** of experience in **RPA**, **Python**, and **GenAI**, focused on building scalable, AI-powered solutions that drive real business impact.

---

## 🛠️ Tools & Technologies
- 🧠 **GenAI & OpenAI Integration**
- 🤖 **Automation Anywhere A360**, **UiPath Studio & Orchestrator**
- 🐍 **Python** (Django, Regex), **VBScript**
- 🗃️ **SQL Server**, **SAP Automation**, **Excel Automation**
- 🌐 **HTML**, **CSS**, **JavaScript**
- 📊 **Power BI**

---

## 📂 Featured Projects

### 🔹 Intelligent Incident Automation System (Oracle)
- **Tools:** UiPath, Python, GenAI  
- 🚀 Reduced manual effort by 70% through intelligent incident classification and touchless automation

### 🔹 Intercompany Reconciliation Tool
- **Tools:** Automation Anywhere, Django, Power BI  
- 💡 Saved 4 FTEs and reduced reconciliation time from 7 days to near real-time

### 🔹 Duplicate Invoice Identification
- **Tools:** UiPath, SAP, SQL  
- 💰 Prevented ₹60 crore in fraudulent payments through automated invoice validation

---

## 📄 Resume
📥 [Download My Resume](https://github.com/your-username/your-repo-name/raw/main/VijayChavdaResume.pdf)

---

## 📫 Contact Me
- 📧 Email: [vijaychavdap@gmail.com](mailto:vijaychavdap@gmail.com)
- 📞 Phone: +91 9510104254
- 🌐 Portfolio: [vijaychavda.github.io](https://vijaychavda.github.io)
- 💼 LinkedIn: [linkedin.com/in/vijay-chavda-6b8b80216](https://linkedin.com/in/vijay-chavda-6b8b80216)

---

Thanks for visiting! Feel free to explore my projects and reach out if you'd like to collaborate or learn more.
